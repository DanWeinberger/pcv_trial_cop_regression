...
# sources, so the shared serotype-level hierarchy (a[s], b1[s], x_u[s], x_i[s])
# now spans all 12 serotypes across 7 PCV7 + 13-valent studies.
#
#   OUTCOME (ragged, study-specific fixed effect delta[k]):
#     whitney      -- Whitney IPD (NCKP surveillance, US), PCV7            [reference]
...
#
# IMPORTANT CAVEAT -- disconnected serotype sets: the 7 PCV7 serotypes and the
# 5 PCV13-additional serotypes never co-occur in the same outcome study, so
# the design is NOT fully connected (no serotype bridges the two groups). The
# reference study (whitney, delta[1] <- 0) sits in the PCV7 component, so
# a[s]/delta[k] for the two PCV13-additional outcome studies (andrews_pcv13,
# vdl_pcv13) are anchored only INDIRECTLY, through the shared hyperprior on
# a[s] (mu_a, tau_a) -- not through direct data linkage, the way the PCV7
# component is anchored to the reference study. In practice this mainly
# widens uncertainty on the ABSOLUTE baseline (a[s], delta[k]) for the
# PCV13-additional block; the relative delta[vdl_pcv13] - delta[andrews_pcv13]
# *is* directly identified (both studies report the same 5 serotypes, so a[s]
# cancels in that comparison). The serotype slope b1[s] -- the actual
# correlate-of-protection quantity -- is estimated the same way it already is
# in the single-study RE model (hierarchical shrinkage across serotypes, each
# serotype informed by only 2 arms per study); check diagnostics.pdf /
# posterior_summary.csv for wide credible intervals or poor mixing on
# a[]/delta[] before over-interpreting those specific parameters.
# =====================================================================
